raw <- read.csv("chum_rs_raw.csv", header=TRUE)
model <- read.csv("chum_rs_model.csv", header=TRUE)
...
chum_graph <- g + annotate("text",  x=1.4, y = 0.035, label = "Chum", vjust=1, hjust=1, size=7)
