...
write.csv(x=coho.datap, file = "coho_predator.csv")
...
#coho <- coho[ which(coho$Coho_total < 300),]
...
# 12.4 and 10
# for longer fit, do 30 and 10

for (i in 1:4){
  data <- coho[ which(coho$Coho_total >= (i-1)*12.5 & coho$Coho_total < i*12.5), ]
...
mod1 <- glm(Injury ~ Coho_total, coho, family = "binomial")
mod2 <- glm(Injury ~ Coho_total + Ocean_age, coho, family = "binomial")

mod3 <- glm(Injury ~ Coho_total + Length_TSFT_mm, coho, family = "binomial")
mod3a <- glm(Injury ~ Coho_total * Length_TSFT_mm, coho, family = "binomial")

mod5 <- glm(Injury ~ Coho_total, coho, family = "binomial")
mod6 <- glm(Injury ~ Coho_total + Length_TSFT_mm + Sex, coho, family = "binomial")
mod7 <- glm(Injury ~ Coho_total + Ocean_age + Sex + Set_time, coho, family = "binomial")
mod8 <- glm(Injury ~ Coho_total + Ocean_age + Set_time, coho, family = "binomial")

AIC(mod1, mod2, mod3, mod3a, mod4, mod5, mod6, mod7, mod8)

# predict with species group size
pred_interval <- seq(min(bin.x),max(bin.x),1)
example <- data.frame(Coho_total=pred_interval, Length_TSFT_mm=rep(600, length(pred_interval)))
pred1 <- predict(mod3a, newdata=example, re.form=NA, type="response")
plot(pred1, type="l", ylab="Prob(predator injury)", xlab="Group Size", xlim=c(0,200))

pred <- predict(mod3a, newdata=example, type="link", se.fit=TRUE)
...
coho.graph <- g + annotate("text",  x=max(bin.x), y = 0.035, label = "Coho", vjust=1, hjust=1, size=7)
